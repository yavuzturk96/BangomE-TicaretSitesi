    	...
    	if($('.product-grid').length)
    	{
    		$('.product-grid').isotope({
	            ...
	            getSortData: {
	            	...
	            	{
	            		var priceEle = $(itemElement).find('.product_price').text().replace( ' TL', '' );
	        ...
	        filterButton.on('click', function()
	        {
	        	$('.product-grid').isotope({
		            filter: function()
		            {
			        	...
			        	var priceMin = parseFloat(priceRange.split(' TL -')[0].replace(' TL', ''));
                        var priceMax = parseFloat(priceRange.split(' TL -')[1].replace(' TL', ''));
                        var itemPrice = $(this).find('.product_price').clone().children().remove().end().text().replace(',', '.').replace(' TL', '');
